...
const float latWayPoint[] = {38.690660, 38.690831}; 
const float lngWayPoint[] = {-9.294800, -9.294116};
float latStart;
float lngStart;
...
void autonomousAction(){
  ...
  float distanceToLine = distanceToProjectPoint(latStart, lngStart, latWayPoint[currentWayPoint], lngWayPoint[currentWayPoint], currentLat, currentLng);
  if (distanceToTarget <= 7.5 ){
    appendFile(SD, "/tiago.txt", "DESTINATION REACHED\n");
    currentWayPoint +=1; // Progress to next waypoint
    latStart = currentLat;
    lngStart = currentLng;
  }
  if (distanceToLine >= 5){ // If too far from the optimal line, change the desired course
    latStart = currentLat;
    lngStart = currentLng;
    desiredHeadingToTarget = gps.courseTo(latStart, lngStart, latWayPoint[currentWayPoint], lngWayPoint[currentWayPoint]); // desired Heading to get from starting position to current waypoint
  }
  ...
  float autoWinchAngle = GetWinchControlAction(windReading);
  int autoWinchPulseWidth = ConvertWinchToPWM(autoWinchAngle);
  SetWinchPosition(autoWinchPulseWidth);

  dataMessage = String(Time) + ", " + String(currentLat, 6) + ", " + String(currentLng, 6) + ", " + String(currentHeading) + ", " + String(desiredHeadingToTarget) + ", " + String(headingError) + ", " + String(Roll) + "," + String(windReading) + "," + String(windDirection) + ", " + String(autoWinchAngle) + ", " + String(autoRudderAngle) + ", " + String(distanceToTarget) + "\r\n";
...
void nanoMessageRead(){
  ...
  if (comma1Index != -1 && comma2Index != -1 && comma3Index != -1) { 
    ...
    int winchVal = winchValueStr.toInt();
    if ( winchVal > 1000 && winchVal < 2500){
      winchPulseWidth = winchVal;
    }
    int rudderVal = rudderValueStr.toInt();
    if ( rudderVal > 1000 && rudderVal < 2500){
      rudderPulseWidth = rudderVal;
    }
    int TempWind = windReadingStr.toInt();
    if (TempWind != 0 && TempWind<=360 && abs(Roll) < 25){ // WindVane data sometimes comes wrongly parsed - this filters those outliers (0.0 values and values greather than 360). Also only takes wind reading if roll is low
...
void setup() {
  ...
  if(!file) {
    writeFile(SD, "/tiago.txt", "Time, Latitude, Longitude, currentHeading, desiredHeading, headingError, Roll, WindReading, WindDirection, autoWinchAngle, autoRudderAngle, distanceToTarget \r\n");
  }
  file.close();
  ...
  // Build the Rudder and Winch Fuzzy Controllers
  RudderControllerSetup();
  WinchControllerSetup();
}
